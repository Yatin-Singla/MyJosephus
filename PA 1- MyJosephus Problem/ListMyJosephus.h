...
public:
	...
	//explicit ListMyJosephus(int size = 0, int N = 0, int M = 0) :size(size), N(N), M(M) {};
	...
	explicit ListMyJosephus(int N = 0, int M = 0) :N(N), M(M) 
	{
		size = N;
		Passes = M;
...
private:
	...
	int N;		// the number of people at the start of the game. CONST
	int M;		// the interval of elimination; i.e., number of positions to skip before next elimination CONST
	int Passes; //remaining passes to be made till the person holding the potato is eliminated
